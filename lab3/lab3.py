...
def trainBoost(base_classifier, X, labels, T=10):
    ...
    for i_iter in range(0, T):
        ...
        err = 0
        for jdx in np.unique(labels):
            idx = np.where(jdx == vote)[0]
            err += np.sum(np.transpose(wCur[idx]) * (1-(labels[idx] == jdx)))
        ...
        for i in range(Npts):
            if vote[i] == labels[i]:
                wCur[i] = wPre[i]  * np.exp(-1*alpha)
            else:
                wCur[i] = wPre[i]  * np.exp(alpha)
        wCur = wCur / np.sum(wCur)

    # print("T wCur: " + str(np.sum(wCur)))
